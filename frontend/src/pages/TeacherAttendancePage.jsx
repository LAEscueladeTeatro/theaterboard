...
// Opciones de estado para el modal en modo histórico
const HISTORIC_STATUS_OPTIONS = [
  { value: 'PUNTUAL', label: 'Puntual' },
  { value: 'A_TIEMPO', label: 'A Tiempo' },
  { value: 'TARDANZA_JUSTIFICADA', label: 'Tardanza Justificada' },
  { value: 'TARDANZA_INJUSTIFICADA', label: 'Tardanza Injustificada' },
  // Considerar añadir AUSENCIAS si se desea registrar/modificar ausencias individuales fuera del flujo de "cierre"
];
...
const TeacherAttendancePage = ({ selectedDate: historicDateProp }) => {
  ...
  // Estados para el modal de Estatus (antes tardanza)
  const [statusModalOpen, setStatusModalOpen] = useState(false);
  const [statusModalStudent, setStatusModalStudent] = useState(null);
  const [statusModalSelectedStatus, setStatusModalSelectedStatus] = useState(''); // Almacenará PUNTUAL, A_TIEMPO, etc.
  ...
  useEffect(() => {
    const newDate = historicDateProp || getTodayPeruDateString();
    setDateForOperations(newDate);
  ...
  useEffect(() => {
    if (!searchTerm) { setStudents(allStudents); return; }
    const lowerSearchTerm = searchTerm.toLowerCase();
    const filtered = allStudents.filter(student => student.full_name.toLowerCase().includes(lowerSearchTerm) || student.id.toLowerCase().includes(lowerSearchTerm));
    setStudents(filtered);
  }, [searchTerm, allStudents]);

  const handleAttendanceChange = (studentId, field, value) => {
    setAttendanceData(prev => ({ ...prev, [studentId]: { ...prev[studentId], [field]: value, is_synced: false } }));
  };

  const determineStatusTypeForModal = () => {
    if (historicDateProp) return 'HISTORIC_MODAL'; // Indica que se debe abrir el modal genérico para histórico

    const nowPeru = currentPeruDateTime; // Ya es un objeto Date con la hora de Perú
    const hours = nowPeru.getHours();
    const minutes = nowPeru.getMinutes();

    if (hours < 17) return 'PUNTUAL_DIRECT';
    if (hours === 17 && minutes <= 15) return 'A_TIEMPO_DIRECT';
    return 'TARDANZA_MODAL'; // Para el día actual, si es tarde, abre el modal de tardanza
  };

  const handleOpenStatusModal = (student, existingStatus = '') => {
    setStatusModalStudent(student);
    setStatusModalNotes(attendanceData[student.id]?.notes || '');
    // Si es histórico o se está corrigiendo, pre-seleccionar el estado actual si existe, sino PUNTUAL por defecto para histórico.
    // Si es tardanza del día actual, se pre-selecciona TARDANZA_INJUSTIFICADA.
    let defaultStatus = '';
    if (historicDateProp) {
      defaultStatus = existingStatus || 'PUNTUAL'; // Para nuevo registro histórico, PUNTUAL por defecto
    } else { // Día actual, y es una tardanza
      defaultStatus = existingStatus || 'TARDANZA_INJUSTIFICADA'; // Para tardanza del día
    }
    setStatusModalSelectedStatus(existingStatus || defaultStatus);
  ...
  const handleCloseStatusModal = () => {
    ...
    setStatusModalSelectedStatus('');
    setStatusModalNotes('');
  };

  const handleSubmitStatusModal = () => {
    if (!statusModalStudent || !statusModalSelectedStatus) {
      alert("Por favor, seleccione un estado de asistencia.");
      return;
    }
    saveAttendanceRecord(statusModalStudent.id, statusModalSelectedStatus, statusModalNotes);
  ...
  const saveAttendanceRecord = async (studentId, status, notes) => {
    try {
      ...
      const response = await axios.post(`${API_URL}/attendance/record`, {
        student_id: studentId, attendance_date: dateForOperations, status: status, notes: notes,
      }, { headers: { 'x-auth-token': token } });
      setAttendanceData(prev => ({ ...prev, [studentId]: { status: status, notes: notes, is_synced: true } }));
      setDailyStatus(prev => {
        ...
        updatedRecords.push({
            ...
            full_name: studentInfo?.full_name || 'Desconocido', nickname: studentInfo?.nickname || '',
            points_earned: response.data.record.points_earned, base_attendance_points: response.data.record.base_attendance_points,
      ...
      alert(`Asistencia para ${studentInfoForAlert?.full_name || studentId} (${dateForOperations}) registrada como ${status}.`);
    } catch (err) {
  ...
  const handleProcessAttendanceClick = (student) => {
    ...
    const currentRecord = dailyStatus.attendance_records.find(r => r.student_id === student.id);

    if (historicDateProp || isRecorded) { // Siempre abrir modal para histórico o para corregir
      handleOpenStatusModal(student, currentRecord?.status);
    } else { // Día actual, nuevo registro
      const statusType = determineStatusTypeForModal();
      if (statusType === 'PUNTUAL_DIRECT') {
        saveAttendanceRecord(student.id, 'PUNTUAL', attendanceData[student.id]?.notes || '');
      } else if (statusType === 'A_TIEMPO_DIRECT') {
        saveAttendanceRecord(student.id, 'A_TIEMPO', attendanceData[student.id]?.notes || '');
      } else { // TARDANZA_MODAL
        handleOpenStatusModal(student, 'TARDANZA_INJUSTIFICADA'); // Pre-selecciona T. Injustificada para tardanza del día
      }
    }
  };

  const handleApplyEarlyBonus = async () => {
    if (historicDateProp) { alert("El bono madrugador solo se puede aplicar en la fecha actual."); return; }
    if (!selectedStudentForBonus) { alert("Por favor, seleccione un estudiante para otorgar el bono."); return; }
    if (dailyStatus.bonus_awarded_today) { alert(`El bono madrugador ya fue otorgado a ${dailyStatus.bonus_awarded_today.bonus_student_name}.`); return; }
    try {
      const token = getToken();
      const response = await axios.post(`${API_URL}/attendance/early-bonus`, {
        student_id: selectedStudentForBonus, bonus_date: dateForOperations,
      }, { headers: { 'x-auth-token': token } });
      setDailyStatus(prev => ({ ...prev, bonus_awarded_today: { ...response.data.bonus_record, bonus_student_name: allStudents.find(s => s.id === response.data.bonus_record.student_id)?.full_name || response.data.bonus_record.student_id }}));
      setSelectedStudentForBonus('');
      ...
      setError(err.response?.data?.message || err.message || 'Error al aplicar el bono.');
      alert(`Error al aplicar bono: ${err.response?.data?.message || err.message}`);
    }
  };
  ...
  const handleOpenCloseAttendanceModal = () => {
    ...
    absent.forEach(student => { initialJustifications[student.id] = { is_justified: false, notes: '' }; });
  ...
  const handleAbsentJustificationChange = (studentId, field, value) => {
    setAbsentJustifications(prev => ({ ...prev, [studentId]: { ...prev[studentId], [field]: value } }));
  };

  const handleSubmitCloseAttendance = async () => {
    const justificationsPayload = Object.entries(absentJustifications).map(([student_id, data]) => ({ student_id, is_justified: data.is_justified, notes: data.notes }));
    try {
      const token = getToken();
      await axios.post(`${API_URL}/attendance/close`, {
        attendance_date: dateForOperations, absent_students_justifications: justificationsPayload,
      }, { headers: { 'x-auth-token': token } });
      ...
      updatedDailyStatusResponse.data.attendance_records.forEach(record => {
        if (!newAttendanceData[record.student_id] || record.status.startsWith("AUSENCIA")) {
             newAttendanceData[record.student_id] = { status: record.status, notes: record.notes || '', is_synced: true };
        }
      });
      setAttendanceData(newAttendanceData);
      alert("Cierre de asistencia procesado.");
      setCloseAttendanceModalOpen(false);
    } catch (err) {
      console.error("Error closing attendance:", err);
      setError(err.response?.data?.message || err.message || 'Error al cerrar asistencia.');
  ...
  const renderStatusModal = () => {
    ...
    const modalTitle = `Registrar/Modificar Estado para: ${statusModalStudent.full_name} (${statusModalStudent.id}) - Fecha: ${dateForOperations}`;
      ...
      <div className="modal-overlay">
        <div className="modal-content">
          ...
          <div>
            <label>Estado:</label>
            <select
              value={statusModalSelectedStatus}
              onChange={(e) => setStatusModalSelectedStatus(e.target.value)}
              style={{ marginLeft: '10px', padding: '5px' }}
            >
              <option value="" disabled>Seleccione un estado</option>
              {isHistoric ? (
                HISTORIC_STATUS_OPTIONS.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)
              ) : ( // Día actual, y es una tardanza (otras opciones se manejan directo)
                <>
                  <option value="TARDANZA_JUSTIFICADA">Tardanza Justificada</option>
                  <option value="TARDANZA_INJUSTIFICADA">Tardanza Injustificada</option>
                </>
              )}
            </select>
          </div>
          <div style={{ marginTop: '10px' }}>
            <label htmlFor="statusModalNotes">Notas:</label>
            <textarea id="statusModalNotes" value={statusModalNotes} onChange={(e) => setStatusModalNotes(e.target.value)} rows="3" style={{ width: '90%', marginTop: '5px' }} />
          </div>
          <div style={{ marginTop: '15px' }}>
            <button onClick={handleSubmitStatusModal}>Guardar Estado</button>
            <button onClick={handleCloseStatusModal} style={{ marginLeft: '10px' }}>Cancelar</button>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="modal-overlay"><div className="modal-content">
          ...
          {absentStudentsForModal.length === 0 ? (<p>Todos los estudiantes ya tienen un registro.</p>) : (
            <table><thead><tr><th>Estudiante</th><th>Justificada</th><th>Notas</th></tr></thead><tbody>
  ...
  return (
    <div>
      ...
      <div><input type="text" placeholder="Buscar estudiante por ID o nombre..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
      <div style={{ margin: '20px 0' }}>
        <h4>Bono Madrugador</h4>
        {dailyStatus.bonus_awarded_today ? (
          <p style={{ color: 'green' }}>Bono Madrugador ya otorgado a: {dailyStatus.bonus_awarded_today.bonus_student_name} ({dailyStatus.bonus_awarded_today.student_id}) por +{dailyStatus.bonus_awarded_today.points_awarded} puntos.</p>
        ) : (
          <div>
            <select value={selectedStudentForBonus} onChange={(e) => setSelectedStudentForBonus(e.target.value)} disabled={!!dailyStatus.bonus_awarded_today || isAttendanceEffectivelyClosed || historicDateProp}>
            ...
            <button onClick={handleApplyEarlyBonus} disabled={!selectedStudentForBonus || !!dailyStatus.bonus_awarded_today || isAttendanceEffectivelyClosed || historicDateProp} style={{ marginLeft: '10px' }}>Otorgar Bono Madrugador</button>
          </div>
        )}
              ...
              <tr key={student.id}>
                ...
                <td>
                  {isRecorded && currentStudentAttendance.status.startsWith('AUSENCIA') ? (<span style={{ color: 'red' }}>Ausente (Cierre Realizado)</span>) : (
                    <button onClick={() => handleProcessAttendanceClick(student)} disabled={statusModalOpen || isAttendanceEffectivelyClosed}>
                      {isRecorded ? 'Corregir Asistencia' : (historicDateProp ? 'Registrar Estado' : 'Marcar Asistencia')}
                    </button>
                  )}
                </td>
                <td><input type="text" value={currentStudentAttendance.notes} onChange={(e) => handleAttendanceChange(student.id, 'notes', e.target.value)} placeholder="Notas..." disabled={(isRecorded && currentStudentAttendance.status.startsWith('AUSENCIA')) || isAttendanceEffectivelyClosed} /></td>
              </tr>);
          })}
        </tbody>
      </table>
